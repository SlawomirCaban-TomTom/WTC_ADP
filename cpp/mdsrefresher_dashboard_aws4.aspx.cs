...
{
    public partial class mdsrefresher_dashboard_aws4 : System.Web.UI.Page
    {
        ...
        protected void fill_grid()
        {
            ...
            query = "select ((finished_task_count::numeric+processing_task_count::numeric-failed_task_count::numeric)/total_task_count::numeric)::numeric(3,2) as progress,* from mdscontentrefresher_mcrcore04_r2.refresh_job where job_name in ('" + arg1 + "','" + arg2 + "') order by created desc";

            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["mdsrefresheraws4"].ConnectionString);
        ...
        protected void fill_groupid_details(string groupid)
        {
            ...
            query = "select rj.zone,rj.group_id,rj.created,t.* from mdscontentrefresher_mcrcore04_r2.groupidstats('{" + groupid + "}'::uuid[]) as t(job_id uuid,status varchar,cnt bigint,descr text) join mdscontentrefresher_mcrcore04_r2.refresh_job rj on rj.job_id=t.job_id where dry_run=false and status not in (" + argstatus + ") and descr in (" + argdescr + ") order by created,zone,rj.job_id";
