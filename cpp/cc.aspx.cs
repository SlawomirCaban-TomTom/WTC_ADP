...
namespace TomTom_Info_Page.cpp
{
    public partial class cc : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                lbl_err.Text = "user not alowed: " + User.Identity.Name.ToLower();
        ...
        protected void Unnamed4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import"].ConnectionString);
                DataTable temp = new DataTable();
                string query = "SELECT count(tool_name) from committer_config_rprod_cpp_r2.tool_config where tool_name = '" + tb_new_project.Text + "' and destination='PROCESSING'";
                string insert1 = "insert into committer_config_rprod_cpp_r2.tool_config values('" + tb_new_project.Text + "','ASYNC','PROCESSING',"+ dd_priority.SelectedValue + ")";
                string insert2 = "insert into committer_config_rprod_cpp_r2.tool_config values('" + tb_new_project.Text + "','SYNC','PROCESSING'," + dd_priority.SelectedValue + ")";
                string update =" update committer_config_rprod_cpp_r2.tool_config set priority ="+dd_priority.SelectedValue+" where tool_name = '" + tb_new_project.Text + "' and destination='PROCESSING'";
        ...
        {
            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import"].ConnectionString);
            string update = "update committer_config_rprod_cpp_r2.tool_config set priority =" + new_prio + " where tool_name = '" + t_name + "' and destination='PROCESSING'";
        ...
        {
            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import"].ConnectionString);
            string update = "delete from committer_config_rprod_cpp_r2.tool_config where tool_name = '" + t_name + "' and destination='PROCESSING'";
            NpgsqlCommand cmd = new NpgsqlCommand(update, conn);
            try
            {
                conn.Open();
        ...
        protected void save_history(string project, string priority, string action)
        {
            ...
            string update = "insert into cc_history (username,action,project,priority,activity_time,env) values ('" + User.Identity.Name.ToLower() + "','"+action+"','"+project+"',"+priority+",'"+DateTime.Now.ToString()+"','RPROD')";
