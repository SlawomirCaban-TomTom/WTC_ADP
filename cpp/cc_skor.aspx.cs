...
{
    public partial class cc_skor : System.Web.UI.Page
    {
        ...
        {
            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import_skor"].ConnectionString);
            DataTable temp = new DataTable();
            string query = "SELECT distinct tool_name \"Tool Name\",priority \"Priority\" from committer_config_skorea_cpp_r2.tool_config where destination='PROCESSING' order by priority,tool_name asc";
        ...
        protected void Unnamed4_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import_skor"].ConnectionString);
                DataTable temp = new DataTable();
                string query = "SELECT count(tool_name) from committer_config_skorea_cpp_r2.tool_config where tool_name = '" + tb_new_project.Text + "' and destination='PROCESSING'";
                string insert1 = "insert into committer_config_skorea_cpp_r2.tool_config values('" + tb_new_project.Text + "','ASYNC','PROCESSING',"+ dd_priority.SelectedValue + ")";
                string insert2 = "insert into committer_config_skorea_cpp_r2.tool_config values('" + tb_new_project.Text + "','SYNC','PROCESSING'," + dd_priority.SelectedValue + ")";
                string update = " update committer_config_skorea_cpp_r2.tool_config set priority =" + dd_priority.SelectedValue+" where tool_name = '" + tb_new_project.Text + "' and destination='PROCESSING'";
        ...
        {
            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import_skor"].ConnectionString);
            string update = "update committer_config_skorea_cpp_r2.tool_config set priority =" + new_prio + " where tool_name = '" + t_name + "' and destination='PROCESSING'";
        ...
        {
            NpgsqlConnection conn = new NpgsqlConnection(ConfigurationManager.ConnectionStrings["tool_import_skor"].ConnectionString);
            string update = "delete from committer_config_skorea_cpp_r2.tool_config where tool_name = '" + t_name + "' and destination='PROCESSING'";
            NpgsqlCommand cmd = new NpgsqlCommand(update, conn);
            try
            {
                conn.Open();
        ...
        protected void save_history(string project, string priority, string action)
        {
            ...
            string update = "insert into cc_history (username,action,project,priority,activity_time,env) values ('" + User.Identity.Name.ToLower() + "','"+action+"','"+project+"',"+priority+",'"+DateTime.Now.ToString()+"','SKOR')";
